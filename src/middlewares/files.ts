...
// TEMP import { FireBase } from 'firebase';
...
export class FileMiddleware {
  ...
  // Otra opción es subir desde el front directamente a FireBase la imagen, sin guardarla en mi servidor. Es una alternativa a Multer.
  ...
  saveImage = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // TERMP const firebase = new FireBase();
      // TEMP const backUpImage = await firebase.uploadFile(userImage);

      req.body[req.file.fieldname] = {
        urlOriginal: req.file.originalname,
        url: imagePath,
        // TEMP urlBackUp: backUpImage,
